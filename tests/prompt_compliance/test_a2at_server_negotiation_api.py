"""Negotiation content API of the server facade (port of Java ``A2ATServerNegotiationApiTest``).

The server facade exposes the same twelve negotiation content methods and template queries as the
client facade over the same shared service, so this suite pins the symmetric surface: the
from-data legs render the Java golden fixtures byte-for-byte against the packaged templates, the
from-text and validation legs run against a scripted LLM client, the raw string template URI
boundary fails fast on every method (D16), every method delegates to the matching service method
with the typed URI, and the deprecation shim round (D1) warns on the three legacy state-machine
facade methods and on the import of every old negotiation demo package.
"""
...
def build_server(env_path: Path, llm_client: object | None = None) -> A2ATServer:
...
def ending_data(conclusion: NegotiationConclusion) -> NegotiationEndingData:
    ...
    return NegotiationEndingData(
        ...
        InformationProposeContent([NegotiationItem("节能区域", "松山湖")], None).with_conclusion(conclusion)
        if False
        else _ending_content(conclusion),
    )


def _ending_content(conclusion: NegotiationConclusion) -> Any:
    """The information ending content carrying one delivered item."""
    from a2a_t.negotiation.content import InformationEndingContent

    return InformationEndingContent(conclusion, [NegotiationItem("节能区域", "松山湖")])
...
def test_from_data_generates_the_java_golden_message(golden_case: GoldenCase, language: str, tmp_path: Path) -> None:
    server = build_server(write_env(tmp_path, language))
    ...
    if golden_case.performative is NegotiationPerformative.PROPOSE:
        ...
        method = server.generate_negotiation_propose_prompt_from_data
    elif golden_case.performative is NegotiationPerformative.ACCEPT:
        data = NegotiationEndingData(context, content)
        method = server.generate_negotiation_accept_prompt_from_data
    elif golden_case.performative is NegotiationPerformative.REJECT:
        data = NegotiationEndingData(context, content)
        method = server.generate_negotiation_reject_prompt_from_data
    else:
        data = NegotiationAbortData(context, content)
        method = server.generate_negotiation_abort_prompt_from_data
...
@pytest.mark.parametrize("language", LANGUAGES)
def test_from_data_abort_message_renders_the_termination_sections(language: str, tmp_path: Path) -> None:
    server = build_server(write_env(tmp_path, language))

    result = server.generate_negotiation_abort_prompt_from_data(abort_data(), NEGOTIATION_ABORT_URI)
    ...
    assert result.build_metadata_content()["negotiationContext"] == {
        "id": SESSION_ID,
        "round": 5,
        "maxRounds": 5,
        "performative": "ABORT",
...
_FROM_TEXT_CASES: tuple[tuple[str, str, Callable[[A2ATServer], Any], str], ...] = (
    (
        "propose",
        '{"items":[{"name":"接入端口名称","value":"举例：P533-珠江旧城-PTN3900-23-TPA1EG24-1"}],"relationship":null}',
        lambda server: server.generate_negotiation_propose_prompt_from_text(
            "请提供接入端口名称。",
    ...
    (
        ...
        lambda server: server.generate_negotiation_accept_prompt_from_text(
            "同意并接入端口名称。",
    ...
    (
        ...
        lambda server: server.generate_negotiation_reject_prompt_from_text(
            "无法提供接入端口名称。",
    ...
    (
        ...
        lambda server: server.generate_negotiation_abort_prompt_from_text(
            "已达到协商轮次上限，终止协商。",
            NegotiationContext(SESSION_ID, 5, 5, NegotiationPerformative.ABORT),
            NEGOTIATION_ABORT_URI,
        ),
...
def test_from_text_generates_after_one_llm_extraction_call(
    name: str, payload: str, call: Callable[[A2ATServer], Any], expected_fragment: str, tmp_path: Path
) -> None:
    llm = ScriptedLlmClient(payload)
    server = build_server(write_env(tmp_path, "zh-CN"), llm)
...
def test_from_text_rejects_a_wrong_family_template_uri(tmp_path: Path) -> None:
    server = build_server(write_env(tmp_path, "zh-CN"), ScriptedLlmClient("{}"))

    with pytest.raises(ValueError):
        server.generate_negotiation_propose_prompt_from_text(
            "请提供接入端口名称。",
            NegotiationContext(SESSION_ID, 1, 5, NegotiationPerformative.PROPOSE),
            NEGOTIATION_ABORT_URI,
        )
...
_VALIDATE_CASES: tuple[tuple[str, str, Callable[[A2ATServer], str], Any, str, Any], ...] = (
    (
        "propose",
        '{"semantic_verdict":true,"negotiation_type":"information","errors":[],"params":{"energyRegion":"松山湖"}}',
        lambda server: (
            server.generate_negotiation_propose_prompt_from_data(
    ...
    (
        ...
        '{"semantic_verdict":true,"negotiation_type":"information","errors":[],"params":{"energyRegion":"松山湖"}}',
        lambda server: (
            server.generate_negotiation_accept_prompt_from_data(
    ...
    (
        ...
        lambda server: (
            server.generate_negotiation_reject_prompt_from_data(
    ...
    (
        ...
        lambda server: (
            server.generate_negotiation_abort_prompt_from_data(abort_data(), NEGOTIATION_ABORT_URI).prompt_text or ""
        ),
        NegotiationContext(SESSION_ID, 5, 5, NegotiationPerformative.ABORT),
        NEGOTIATION_ABORT_URI,
...
_VALIDATE_METHODS: dict[str, str] = {
...
def test_validate_extracts_params_from_the_generated_message(
    name: str,
    payload: str,
    message_builder: Callable[[A2ATServer], str],
    ...
    llm = ScriptedLlmClient(payload)
    server = build_server(write_env(tmp_path, "zh-CN"), llm)
    message = message_builder(server)
    schema = {"type": "object", "properties": {key: {"type": "string"} for key in params}}

    filled = getattr(server, _VALIDATE_METHODS[name])(message, context, schema, template_uri)
...
def test_validate_reports_a_null_context_as_not_being_a_negotiation_message(tmp_path: Path) -> None:
    ...
    with pytest.raises(Exception):
        server.validate_propose_prompt_and_data_filling(
...
def _boundary_calls(server: A2ATServer) -> dict[str, Callable[..., Any]]:
    ...
    return {
        "generate_negotiation_propose_prompt_from_data": lambda uri: (
            server.generate_negotiation_propose_prompt_from_data(  # noqa: E501
                propose_data(), uri
            )
        ),
        "generate_negotiation_accept_prompt_from_data": lambda uri: server.generate_negotiation_accept_prompt_from_data(  # noqa: E501
            ending_data(NegotiationConclusion.ACCEPT), uri
        ),
        "generate_negotiation_reject_prompt_from_data": lambda uri: server.generate_negotiation_reject_prompt_from_data(  # noqa: E501
            ending_data(NegotiationConclusion.REJECT), uri
        ),
        "generate_negotiation_abort_prompt_from_data": lambda uri: server.generate_negotiation_abort_prompt_from_data(
            abort_data(), uri
        ),
        "generate_negotiation_propose_prompt_from_text": lambda uri: (
            server.generate_negotiation_propose_prompt_from_text("请提供接入端口名称。", propose_context, uri)
        ),
        "generate_negotiation_accept_prompt_from_text": lambda uri: server.generate_negotiation_accept_prompt_from_text(
            "同意。", NegotiationContext(SESSION_ID, 2, 5, NegotiationPerformative.ACCEPT), uri
        ),
        "generate_negotiation_reject_prompt_from_text": lambda uri: server.generate_negotiation_reject_prompt_from_text(
            "无法提供。", NegotiationContext(SESSION_ID, 2, 5, NegotiationPerformative.REJECT), uri
        ),
        "generate_negotiation_abort_prompt_from_text": lambda uri: server.generate_negotiation_abort_prompt_from_text(
            "终止协商。", abort_context, uri
        ),
        "validate_propose_prompt_and_data_filling": lambda uri: server.validate_propose_prompt_and_data_filling(
            "message", propose_context, {"type": "object"}, uri
        ),
        "validate_accept_prompt_and_data_filling": lambda uri: server.validate_accept_prompt_and_data_filling(
            "message", NegotiationContext(SESSION_ID, 2, 5, NegotiationPerformative.ACCEPT), {"type": "object"}, uri
        ),
        "validate_reject_prompt_and_data_filling": lambda uri: server.validate_reject_prompt_and_data_filling(
            "message", NegotiationContext(SESSION_ID, 2, 5, NegotiationPerformative.REJECT), {"type": "object"}, uri
        ),
        "validate_abort_prompt_and_data_filling": lambda uri: server.validate_abort_prompt_and_data_filling(
...
def test_template_uri_boundary_rejects_malformed_uris(method_name: str, malformed_uri: str, tmp_path: Path) -> None:
    server = build_server(write_env(tmp_path, "zh-CN"))

    with pytest.raises(ValueError, match="Unparseable template URI"):
        _boundary_calls(server)[method_name](malformed_uri)
...
def test_template_uri_boundary_rejects_a_null_uri(method_name: str, tmp_path: Path) -> None:
    ...
    with pytest.raises(TypeError):
        _boundary_calls(server)[method_name](None)
...
def test_template_uri_boundary_rejects_wrong_family_uris(
    ...
    with pytest.raises(ValueError):
        _boundary_calls(server)[method_name](wrong_family_uri)
...
def test_every_negotiation_method_delegates_to_the_content_service(
    ...
    server._negotiation_content_service = recording

    result = getattr(server, facade_method)(*args)
...
def test_template_queries_delegate_to_the_query_service(tmp_path: Path) -> None:
    ...
    server._template_query_service = recording

    assert server.get_prompts() == []
    assert server.get_prompt(INFORMATION_NEGOTIATION_PROPOSE_URI) is None
...
def test_get_prompt_loads_one_bundled_template(template_uri: str, language: str, tmp_path: Path) -> None:
    ...
    template = server.get_prompt(template_uri)
...
def test_get_prompt_answers_none_for_unknown_templates(unknown_uri: str, tmp_path: Path) -> None:
    ...
    assert server.get_prompt(unknown_uri) is None
...
def test_legacy_negotiation_methods_warn_and_still_delegate(
    ...
    server = build_server(write_env(tmp_path, "zh-CN"))
    input = build_input()
    with patch.object(server._negotiation_orchestrator, method, return_value={"legacy": True}) as delegated:
        with pytest.warns(DeprecationWarning, match=f"A2ATServer\\.{method} is deprecated since 1\\.1\\.0"):
            result = (
                getattr(server, method)(input)
                if method != "receive_negotiation"
                else server.receive_negotiation(*input)
            )
...
def test_negotiation_api_surface_is_complete() -> None:
    for method in (*NEGOTIATION_API_METHODS, "get_prompts", "get_prompt"):
        assert callable(getattr(A2ATServer, method, None)), f"A2ATServer is missing {method}"
